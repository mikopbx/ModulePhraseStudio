...
1. Откройте **Модули → Студия фраз (TTS)**. Сразу после включения модуля бинарь Piper (≈ 25 МБ, статически слинкован, под архитектуру АТС) автоматически скачивается в фоне в `db/piper/`, а каталог голосов обновляется с upstream. Вкладка **«Движок»** сама перейдёт в состояние *«установлен»*, как только архив распакуется.
2. Если автоустановка не сработала (нет интернета, air-gapped) — поднять её можно вручную: вкладка **«Движок»** → **«Установить движок»**. Та же кнопка работает как **«Обновить движок»**: при повторном нажатии бинарь перекачивается даже когда он уже установлен (под капотом отправляется `force=true`).
3. На вкладке **«Голоса»** найдите нужные языки и нажмите **«Загрузить»** на каждом. Модели весят 30–60 МБ и складываются в `db/voices/`. Загрузка идёт асинхронно: строка сразу показывает спиннер *«Устанавливается…»* и переходит в *«Установлен»* (либо *«Ошибка»* с описанием) по факту завершения фоновой задачи — страницу можно закрыть и вернуться, прогресс не потеряется.
...
| POST   | `engine:install`                        | Установить бинарь Piper. Передайте `{"force":true}` для перекачивания (флоу «Обновить движок») |
| DELETE | `engine`                                | Удалить бинарь Piper                      |
| GET    | `voices`                                | Каталог + флаг установки. Опциональные query: `language=ru-ru`, `installed_only=true` |
| POST   | `voices:install`                        | **Асинхронно.** Ставит загрузку в очередь и возвращает `202 Accepted` с `install_status="installing"` — нужно опрашивать `GET voices`, пока строка не перейдёт в `installed` / `failed` |
...
Записи голосов в `GET voices` несут поля статуса асинхронной установки: `install_status` (`""` / `"installing"` / `"installed"` / `"failed"`) и `install_error` (заполнен только при `failed`). Если строка зависла в `installing` дольше пяти минут, бекенд автоматически переводит её в `failed` с синтетическим текстом таймаута — цикл опроса завершится без ручного вмешательства.
...
Пример — установка голоса с опросом готовности:

```bash
curl -X POST -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"voice_id":"ru_RU-irina-medium"}' \
  https://pbx.example.com/pbxcore/api/v3/module-phrase-studio/voices:install
# → 202 {"voice_id":"ru_RU-irina-medium","install_status":"installing","queued":true,...}

curl -H "Authorization: Bearer $TOKEN" \
  "https://pbx.example.com/pbxcore/api/v3/module-phrase-studio/voices?installed_only=true"
# → опрашивайте, пока строка не покажет install_status="installed"
```
